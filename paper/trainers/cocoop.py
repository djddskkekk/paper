...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image, label=None, domain=None):
        ...
        prompts_domain = self.prompt_learner_domain(image_features)
        ...
        logits_domain = []
        for pts_i, imf_i in zip(prompts_domain, image_features):
            text_features_domain = self.text_encoder(pts_i, tokenized_prompts_domain)
            text_features_domain = text_features_domain / text_features_domain.norm(dim=-1, keepdim=True)
            l_i = logit_scale * imf_i @ text_features_domain.t()
            logits_domain.append(l_i)
        logits_domain = torch.stack(logits_domain)

        if self.prompt_learner.training:
            return F.cross_entropy(logits, label), F.cross_entropy(logits_domain, domain)

        return logits, logits_domain
...
class CoCoOp(TrainerX):
    ...
    def forward_backward(self, batch):
        ...
        if prec == "amp":
            ...
            # optim.step()
            lambda_loss = 1.
            loss1, loss2 = model(image, label, domain)
